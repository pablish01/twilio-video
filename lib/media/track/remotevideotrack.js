...
const devicePixelRatioMonitor = require('../../util/devicepixelratiomonitor.js');
...
class RemoteVideoTrack extends RemoteMediaVideoTrack {
  ...
  attach(el) {
    ...
    if (this._clientTrackSwitchOffControl === 'auto') {
      ...
      this._devicePixelRatioMonitorCleanup = setupDevicePixelMonitor(this);
    }
  ...
  detach(el) {
    ...
    if (this._attachments.size === 0) {
      ...
      if (this._devicePixelRatioMonitorCleanup) {
        this._devicePixelRatioMonitorCleanup();
        this._devicePixelRatioMonitorCleanup = null;
      }
    }
...
function setupDevicePixelMonitor(remoteVideoTrack) {
  function onDevicePixelRatioChanged(devicePixelRatio) {
    remoteVideoTrack._devicePixelRatio = devicePixelRatio;
    remoteVideoTrack._log.debug('_devicePixelRatio changed: ', remoteVideoTrack._devicePixelRatio);
    maybeUpdateDimensionHint(remoteVideoTrack);
  }

  remoteVideoTrack._devicePixelRatio = devicePixelRatioMonitor.registerCallback(onDevicePixelRatioChanged);
  return () => {
    devicePixelRatioMonitor.unRegister(onDevicePixelRatioChanged);
  };
}

function setupDocumentVisibilityTurnOff(remoteVideoTrack) {
  function onVisibilityChanged() {
    maybeUpdateEnabledHint(remoteVideoTrack);
  }
...
function maybeUpdateDimensionHint(remoteVideoTrack) {
  ...
  if (totalElements.length > 0) {
    ...
    const renderDimensions = { height: clientHeight * remoteVideoTrack._devicePixelRatio, width: clientWidth * remoteVideoTrack._devicePixelRatio};
